...
(function () {
    ...
    function kinobase(component, _object) {
      ...
      var mirrors = [];
      var mirror_try = 0;
      var search_retry = false;

      if (!prox) {
        mirrors.push(host);
        if (host !== 'https://kinobase.org') mirrors.push('https://kinobase.org');
      }
      var logged_in = !(prox || Lampa.Platform.is('android'));
      var user_agent = Utils.baseUserAgent();
      ...
      this.search = function (_object, kinopoisk_id, data) {
        ...
        network["native"](component.proxyLink(url, prox, prox_enc_page), function (str) {
          ...
          if (!prox && mirrors.length > 1 && mirror_try < mirrors.length - 1) {
            mirror_try++;
            search_retry = true;
            host = mirrors[mirror_try];
            ref = host + '/';
    ...
    function cdnvideohub(component, _object) {
      ...
      function extractItems(sources) {
        if (!sources) return [];
        var items = [];
        /* 4K и 2K перепутаны: mpeg2kUrl это 2K, mpeg4kUrl это 4K */
    ...
    function kodik(component, _object) {
      ...
      var token = Lampa.Storage.get('online_mod_kodik_token', '') + '' || Utils.decodeSecret([124, 125, 1, 86, 90, 64, 12, 123, 108, 59, 122, 125, 82, 3, 90, 23, 90, 122, 60, 110, 43, 123, 84, 3, 91, 71, 88, 112, 111, 57, 122, 121], atob('ZmluZCB5b3VyIG93biB0b2tlbg=='));
    ...
    function component(object) {
      ...
      this.getDefaultQuality = function (qualityMap, defValue) {
        ...
        if (qualityMap) {
          ...
          // ---- AUTO: всегда берём максимально доступное качество ----
          if (!forcedQuality && Lampa.Storage.field('online_mod_auto_quality') === true) {
            for (var _i0 = 0; _i0 < items.length; _i0++) {
              var _item0 = items[_i0];
              if (qualityMap[_item0]) return qualityMap[_item0];
            }

            for (var any in qualityMap) {
              return qualityMap[any];
            }
          }
          ...
          // ---- Если предпочтительного качества нет — всё равно берём максимум ----
          for (var _i2 = 0; _i2 < items.length; _i2++) {
            var _item2 = items[_i2];
            if (qualityMap[_item2]) return qualityMap[_item2];
          }
        }
      ...
      this.emptyForQuery = function (query) {
        ...
        if (Lampa.Storage.field('online_mod_auto_source') === true) {
          var try_next = filter_sources[autosource_attempts];

          if (try_next && try_next !== balanser && sources[try_next]) {
            autosource_attempts++;

            if (autosource_attempts >= filter_sources.length) {
              autosource_attempts = 0;
            }
    ...
    function initStorage() {
      ...
      Lampa.Params.trigger('online_mod_auto_quality', true);
      Lampa.Params.trigger('online_mod_auto_source', true);
      ...
      Lampa.Params.select('online_mod_kodik_token', '', '');
    ...
    function initLang() {
      ...
      Lampa.Lang.add({
        ...
        online_mod_auto_quality: {
          ru: 'Авто-качество (всегда максимальное)',
          uk: 'Авто-якість (завжди максимальна)',
          be: 'Аўта-якасць (заўсёды максімальная)',
          en: 'Auto quality (always maximum)',
          zh: '自动质量（始终最大）'
        },
        online_mod_auto_source: {
          ru: 'Авто-выбор источника (если не найдено)',
          uk: 'Авто-вибір джерела (якщо не знайдено)',
          be: 'Аўта-выбар крыніцы (калі не знойдзена)',
          en: 'Auto source (on empty result)',
          zh: '自动源（当没有结果时）'
        },
        online_mod_kodik_token: {
          ru: 'Kodik token',
          uk: 'Kodik token',
          be: 'Kodik token',
          en: 'Kodik token',
    ...
    function initSettings() {
      ...
      template += "\n        <div class=\"settings-param selector\" data-name=\"online_mod_kodik_token\" data-type=\"input\" placeholder=\"#{settings_cub_not_specified}\">\n            <div class=\"settings-param__name\">#{online_mod_kodik_token}</div>\n            <div class=\"settings-param__value\"></div>\n        </div>";
      ...
      template += "\n        <div class=\"settings-param selector\" data-name=\"online_mod_auto_quality\" data-type=\"toggle\">\n            <div class=\"settings-param__name\">#{online_mod_auto_quality}</div>\n            <div class=\"settings-param__value\"></div>\n        </div>";
      template += "\n        <div class=\"settings-param selector\" data-name=\"online_mod_auto_source\" data-type=\"toggle\">\n            <div class=\"settings-param__name\">#{online_mod_auto_source}</div>\n            <div class=\"settings-param__value\"></div>\n        </div>";
